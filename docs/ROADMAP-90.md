...
| Task | Status | Notes |
|------|--------|-------|
| Vitest `pool: 'forks'` for Windows | ✅ / in progress | Fixes OneDrive path worker timeout locally |
...
1. **Vitest `pool: 'forks'`** — reliable local test runs on Windows (~15 min).
2. **Playwright E2E** — login + availability smoke test (~1 day).
3. **Lazy-load admin routes** — faster load on squad phones (~half day).
