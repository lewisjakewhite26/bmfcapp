...
test.describe('Onboarding', () => {
  ...
  test('admin creates a new invite link', async ({ page }) => {
    ...
    const inviteLink = page.locator('section').filter({
      has: page.getByRole('heading', { name: 'Invite link (optional)' }),
    })
    await expect(inviteLink.getByPlaceholder(/trialist/i)).toBeVisible()
    await inviteLink.getByPlaceholder(/trialist/i).fill('E2E Trialist')
    await inviteLink.getByRole('button', { name: 'Create invite' }).click()
